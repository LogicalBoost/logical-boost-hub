...
export function mapComponentsToSlots(
  ...
  for (const slot of slots) {
    // Business asset slots — never auto-fill from copy components
    if (slot.source === 'business') {
      businessSlots.push(slot.id)
      continue
    }

    // Media slots — never auto-fill
    if (slot.source === 'media') {
      mediaSlots.push(slot.id)
      continue
    }

    // Copy slots — match by contentType
    const available = byType[slot.contentType] || []

    // Populate options for this slot's content type
