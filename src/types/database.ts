...
export type CopyComponentType =
  | 'banner_headline'        // Ad Builder top-slot hook (≤60 chars, audience-tagged)
  | 'headline'
  | 'subheadline'
  | 'primary_text'
  | 'google_headline'
...
// Ad Builder
// ============================================================================
// All ad-builder slots reference rows in copy_components:
//   - bh   -> type = 'banner_headline' (audience-tagged via avatar_ids, ≤60 chars)
//   - body -> type IN ('subheadline','hero_subheadline','proof')
//   - cta  -> type IN ('cta','hero_cta')

export const AD_BODY_SH_TYPES: CopyComponentType[] = ['subheadline', 'hero_subheadline']
export const AD_BODY_PC_TYPES: CopyComponentType[] = ['proof']
export const AD_BODY_TYPES_FLAT: CopyComponentType[] = [...AD_BODY_SH_TYPES, ...AD_BODY_PC_TYPES]
export const AD_CTA_TYPES: CopyComponentType[] = ['cta', 'hero_cta']

export function adBodyGroupCode(type: CopyComponentType): 'SH' | 'PC' | null {
  if (AD_BODY_SH_TYPES.includes(type)) return 'SH'
  if (AD_BODY_PC_TYPES.includes(type)) return 'PC'
  return null
}

// Stable per-client per-type seq for the BH name segment (BH4 = 4th BH for the
// client by created_at). Computed client-side to mirror the DB trigger.
export function bhSeqFor(component: CopyComponent, allComponents: CopyComponent[]): number {
  const pool = allComponents
    .filter(c => c.client_id === component.client_id && c.type === 'banner_headline')
    .sort((a, b) =>
      a.created_at < b.created_at ? -1
      : a.created_at > b.created_at ? 1
      : a.id < b.id ? -1 : 1,
    )
  return pool.findIndex(c => c.id === component.id) + 1
}
...
export interface Ad {
  ...
  offer_id: string
  audience_id: string
  bh_component_id: string             // banner_headlines.id
  body_component_id: string           // copy_components.id  (subheadline / hero_subheadline / proof)
  cta_component_id: string            // copy_components.id  (cta / hero_cta)
  bh_component_version: number
  body_component_version: number | null  // not pinned (copy_components has no version column)
  cta_component_version: number | null
  name: string                         // generated by trigger; e.g. IMMI_UNDOC_BH4-SH3-CTA10
  status: 'approved' | 'denied'
  created_by: string | null
  created_at: string
  updated_at: string
...
export interface BannerAsset {
  ...
  source: 'ai' | 'designer'
  storage_path: string        // path within the client-assets storage bucket
  width: number | null
  height: number | null
  created_by: string | null
  created_at: string
...
// Hard cap for BH content. Soft warning starts at BH_LENGTH_WARN.
export const BH_LENGTH_HARD = 60
export const BH_LENGTH_WARN = 40

// Code format for offers and audiences (matches DB CHECK constraint).
export const CODE_REGEX = /^[A-Z0-9]{2,6}$/
