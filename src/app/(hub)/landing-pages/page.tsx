...
export default function LandingPagesPage() {
  ...
  // Step 4: Form + Phone selection (forms are built in the Forms tab)
  const [selectedFormId, setSelectedFormId] = useState<string | null>(null)
  const [selectedPhoneId, setSelectedPhoneId] = useState<string | null>(null)

  // Tab state: builder vs published pages list — clients default to pages view
  const [activeView, setActiveView] = useState<'builder' | 'pages' | 'forms'>(isClientRole ? 'pages' : 'builder')
  ...
  return (
    <div className="page-container-responsive" style={{ padding: '24px 32px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              background: 'none',
              border: 'none',
              borderBottom: activeView === 'forms' ? '2px solid var(--accent)' : '2px solid transparent',
              color: activeView === 'forms' ? 'var(--accent)' : 'var(--text-muted)',
              cursor: 'pointer',
              marginBottom: -2,
              transition: 'color 0.15s, border-color 0.15s',
              display: 'flex',
      ...
      {/* Forms Tab */}
      {/* ============================================================ */}
      {activeView === 'forms' && !isClientRole && (
        <FormsTab
          client={client}
          forms={forms}
          formWebhooks={formWebhooks}
          refreshForms={refreshForms}
          refreshFormWebhooks={refreshFormWebhooks}
          canEdit={canEdit}
        />
      )}
      ...
      {step === 4 && (
        <BuildStep
          ...
          onGoToForms={() => setActiveView('forms')}
          onBack={() => setStep(3)}
...
function BuildStep({
  ...
  selectedPhoneId: currentPhoneId,
  onFormChange,
  onPhoneChange,
  onGoToForms,
  onBack,
  ...
  clientPhoneNumbers?: Array<{ id: string; phone_number: string; label: string; is_default: boolean }>
  selectedFormId?: string | null
  selectedPhoneId?: string | null
  onFormChange?: (id: string | null) => void
  onPhoneChange?: (id: string | null) => void
  onGoToForms?: () => void
  onBack: () => void
  onPublished?: () => void
}) {
    ...
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      {!buildResult && (
        <div style={{ ...card({ padding: 24 }) }}>
          ...
          {/* Form + Phone Selection */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }} className="grid-2col-responsive">
            <div>
                  ...
                  >
                    ...
                    {formsList.map(f => (
                      <option key={f.id} value={f.id}>
                        {f.name} ({f.form_type === 'multi_step' ? `${f.steps?.length || 0}-step` : 'Standard'} &middot; {f.fields.length} fields)
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <button
                onClick={() => onGoToForms?.()}
                style={{ marginTop: 6, background: 'none', border: 'none', color: 'var(--accent)', fontSize: 12, cursor: 'pointer', padding: 0, fontWeight: 600 }}
              >
                {formsList && formsList.length > 0 ? 'Manage Forms' : 'Create a Form'} &rarr;
              </button>
            </div>
            <div>
              <label style={{ fontSize: 12, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6, display: 'block' }}>
              ...
              {phoneNumbers && phoneNumbers.length > 0 ? (
                <select
                  ...
                  onChange={(e) => onPhoneChange?.(e.target.value || null)}
                >
                  <option value="">No phone number</option>
                  {phoneNumbers.map(pn => (
                    <option key={pn.id} value={pn.id}>
                      {pn.phone_number} &mdash; {pn.label}{pn.is_default ? ' (default)' : ''}
                    </option>
                  ))}
                </select>
              ) : (
                <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
                  No phone numbers. Add in Settings &rarr; Phone Numbers.
                </div>
              )}
            </div>
          </div>
