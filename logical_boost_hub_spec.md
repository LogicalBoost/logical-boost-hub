...
| id | UUID | Primary key |
| client_id | UUID | FK → clients |
| funnel_instance_id | UUID | Nullable. FK → funnel_instances |
| avatar_id | UUID | FK → avatars |
| offer_id | UUID | FK → offers |
| template_id | string | Template used: `clean_authority`, `bold_conversion`, `gap_play`, `aggressive_dr` |
| copy_component_ids | json array | Components used as AI input |
| headline | string | Hero headline |
| subheadline | string | Hero subheadline |
| cta | string | Primary CTA |
| section_data | JSONB | Structured section data (AI-generated JSON, see Section Structure below) |
| page_html | TEXT | Self-contained rendered HTML (generated by template-renderer.ts) |
| brand_kit_snapshot | JSONB | Snapshot of brand kit used at generation time |
| deployed_url | string | Nullable. Public URL after deployment to Supabase Storage |
| deploy_status | string | `draft`, `deployed` |
| status | string | `approved`, `denied` |
| created_at | timestamp | |
| updated_at | timestamp | |
...
AI generates structured JSON (`section_data`) → deterministic renderer (`template-renderer.ts`) produces self-contained HTML (`page_html`). This separation ensures consistent, well-formed HTML while AI controls content and the renderer controls design.

### 4 Premium Templates

| Template | ID | Sections |
|----------|---|----------|
| Clean Authority | `clean_authority` | hero, problem, solution, benefits, proof, faq, final_cta |
| Bold Conversion | `bold_conversion` | hero, urgency_bar, trust_strip, problem, solution, benefits, proof, faq, final_cta |
| Gap Play | `gap_play` | hero, problem, process_steps, benefits, proof, faq, final_cta |
| Aggressive DR | `aggressive_dr` | hero, urgency_bar, trust_strip, benefits, proof, problem, process_steps, proof, faq, final_cta |

### Section Data Structure

Each section has: `id` (e.g. "hero_1"), `type`, `order`, and `content` object. Content shapes per type:

```json
{
  "sections": [
    { "id": "hero_1", "type": "hero", "order": 1, "content": { "headline": "", "subheadline": "", "cta": "", "trust_items": [] } },
    { "id": "problem_1", "type": "problem", "order": 2, "content": { "headline": "", "content": "" } },
    ...
    { "id": "proof_1", "type": "proof", "order": 5, "content": { "headline": "", "testimonials": [{ "quote": "", "author": "", "role": "" }], "stats": [{ "number": "", "label": "" }] } },
...
Additional section types: `urgency_bar` (text), `trust_strip` (items array), `process_steps` (headline + steps array).

### Template Design

All templates use premium dark-mode-first design:
- Gradient mesh backgrounds with animated floating orbs
- Glassmorphism: frosted glass nav and cards (`backdrop-filter: blur`)
- Gradient text headlines (`-webkit-background-clip: text`)
- Scroll-triggered animations via IntersectionObserver
- Fully responsive with mobile hamburger nav
- Self-contained HTML: inline CSS, Google Fonts, minimal JS

### Deployment

Landing page HTML is uploaded to Supabase Storage (`client-assets` bucket) at `{client_id}/pages/{slug}.html`. The public URL is saved to `deployed_url` and status changes to `deployed`.
...
- Landing pages are now built via the dedicated Landing Pages page (see Page 8: Landing Pages)
...
## Page 8: LANDING PAGES

**Purpose:** Build, edit, and deploy high-converting landing pages for each Avatar + Offer combination. Uses a 5-stage pipeline from brand analysis through to live deployment.

### 5-Stage Pipeline

Progress shown as a horizontal tab bar with checkmarks for completed stages:

**Stage 1: Brand Kit**
- Analyzes client website to extract brand colors, fonts, and visual identity
- Stores brand_kit on client record for use in page rendering
- Triggers `analyze-brand-kit` edge function

**Stage 2: Competitive Analysis**
- Shows competitors with websites; allows analyzing their landing pages
- Triggers `analyze-competitor-pages` edge function
- Results stored as competitor_intel records with `ad_type: 'landing_page'`

**Stage 3: Industry Playbook**
- Generates strategic landing page playbook from competitive analysis data
- Produces concept briefs with strategy, above-fold approach, key sections, tone, differentiator
- Triggers `generate-playbook` edge function
- Stored as `landing_page_playbook` on client record

**Stage 4: Concept Pages**
- Displays 4 strategic landing page concepts from the playbook
- Each concept card shows: name, strategy, above-fold approach, key sections, tone, differentiator
- Concepts serve as strategic direction for page generation

**Stage 5: Page Builder**
- Select avatar (sorted by priority) + offer + template
- Click "Generate Landing Page" → AI generates section JSON + renders HTML
- Split-pane UI: section list (left) + iframe preview (right)
- Desktop / Tablet / Mobile preview size toggles
- AI Edit panel: natural language prompts to edit individual sections or full page
- Deploy bar: custom slug input → uploads to Supabase Storage → shows LIVE badge with public URL
- Re-deploy updates existing deployed page
- Lists all previously generated pages for the client

### Key UI Elements
- Pipeline stage tabs with ✓ checkmarks (green when complete, muted when incomplete)
- Builder layout: CSS Grid with 280px sidebar + 1fr preview
- Preview iframe uses `srcDoc` to render self-contained HTML
- Deploy creates public URL: `https://{project}.supabase.co/storage/v1/object/public/client-assets/{client_id}/pages/{slug}.html`
...
7. **Offers**
8. **Competitor Ads**
9. **Landing Pages** (5-stage pipeline: brand kit → competitive → playbook → concepts → builder)
...
**Input:**
- `client_id`, `avatar_id`, `offer_id`, `template_id`
- Optional `concept_brief_index` (references a concept brief from the playbook)
- Client data (business_summary, services, differentiators, trust_signals, tone, brand_kit, logo_url)
- Avatar data (pain_points, motivations, objections, desired_outcome, trigger_events, messaging_style)
- Offer data (headline, subheadline, benefits, proof, FAQ, CTA, conversion_type)
- Approved copy_components (up to 100, used as raw material / inspiration)

**Process (Edge Function: `generate-landing-page`):**
1. Fetch client, avatar, offer, and approved copy_components in parallel
2. Build system prompt with template section schema, copywriter identity, quality/formatting rules
3. Build user message with all business context, avatar details, offer details, grouped copy components, and optional concept brief
4. Call Claude (sonnet, 8192 tokens) to generate structured section JSON
5. Parse JSON response into sections array with meta (headline, subheadline, cta)
6. Render self-contained HTML via `renderLandingPage()` from `template-renderer.ts`
7. Insert `landing_pages` record with `section_data`, `page_html`, `brand_kit_snapshot`, `deploy_status: 'draft'`

**Output:**
- landing_page record with structured section_data JSON + rendered page_html

---

## Workflow 8: EDIT LANDING PAGE SECTION

**Trigger:** User types an edit prompt in the AI Edit panel (Landing Pages page builder)

**Input:**
- `landing_page_id`, `user_prompt`, `edit_scope` (`section` or `full_page`), optional `section_id`

**Process (Edge Function: `edit-landing-page-section`):**
1. Fetch landing page + associated client, avatar, offer for context
2. If section edit: find target section, build single-section edit prompt, call Claude (4096 tokens)
3. If full page edit: build full-page edit prompt with all sections, call Claude (8192 tokens)
4. Parse response, preserve original id/type/order, only update content
5. Re-render full page HTML via `renderLandingPage()`
6. Update landing_pages record with new section_data and page_html

**Output:**
- Updated landing_page record with edited content and re-rendered HTML

---

## Workflow 9-12: LANDING PAGE PIPELINE

The Landing Pages page follows a 5-stage pipeline before building:

1. **Brand Kit Analysis** (`analyze-brand-kit`) — Extract colors, fonts, visual identity from client website
2. **Competitive Analysis** (`analyze-competitor-pages`) — Analyze competitor landing pages for patterns
3. **Industry Playbook** (`generate-playbook`) — Synthesize competitive data into strategic playbook with concept briefs
4. **Concept Pages** — Display 4 strategic landing page concepts from the playbook for review
5. **Page Builder** — Select avatar + offer + template → generate → AI edit → deploy to live URL
